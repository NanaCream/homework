...
kust = int(input('Введите количество кустов на грядке: '))    # N
garden = []
for i in range(kust):
    berries = int(input ("Введите число ягод на кусте: "))
    garden.append(berries)

sum_berries=[]
sum_berries.append(garden[-1]+ garden[0] + garden[1])

for i in range (1,(kust-1)):
    sum_berries.append(garden[i-1] + garden[i] + garden[i+1])

sum_berries.append(garden[0] + garden[-1] + garden[-2])
